...
public class ChatGuiClient extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
       ...
       // names.remove(socketIn.)
        ...
        catButton = new Button("=^._.^=");
        catButton.setDisable(true);
        catButton.setOnAction(e -> sendCat());

        HBox hbox = new HBox();
        hbox.getChildren().addAll(new Label("Message: "), textInput, catButton, sendButton);
    ...
    public void sendCat() {
        sendMessage(new MessageCtoS_Cat());
    }
    ...
    private void sendChatMessage() {
        ...
        String to = ((RadioButton)group.getSelectedToggle()).getText();

        if (to.equals("Everyone")) {
            sendMessage(new MessageCtoS_Chat(msg));
        } else {
            sendMessage(new MessageCtoS_Private(msg, to));
        }
    }
